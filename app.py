...
with tab4:
    ...
    gap_file_tab4 = st.file_uploader("Upload results CSV", type="csv", key="gap_upload_tab4_unique")
    
    df_gap = None
    
    if 'latest_results' in st.session_state and st.session_state.latest_results is not None:
        use_latest_gap = st.checkbox("Use results from Multi-LLM Generator", value=True, key="gap_use_latest_tab4_unique")
        if use_latest_gap:
            df_gap = st.session_state.latest_results.copy()
    elif gap_file_tab4:
        df_gap = pd.read_csv(gap_file_tab4)
